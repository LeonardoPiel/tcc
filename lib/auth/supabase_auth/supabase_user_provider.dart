...
class TccLeoSupabaseUser extends BaseAuthUser {
  TccLeoSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> tccLeoSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = TccLeoSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
